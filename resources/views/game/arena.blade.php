    ...
    <div class="space-y-8">
        <div class="flex flex-wrap items-end justify-between gap-3">
            <div>
                ...
                <p class="mt-1 text-sm text-zinc-400">Выбери сущность, посмотри подходящих соперников и брось вызов.</p>
            </div>
        ...
        @if ($incomingChallenges->isNotEmpty() || $outgoingChallenges->isNotEmpty())
            <section class="grid gap-4 lg:grid-cols-2">
                <div class="rounded-md border border-zinc-800 bg-zinc-900 p-5">
                    <div class="flex items-center justify-between gap-3">
                        <h2 class="text-xl font-semibold text-white">Входящие вызовы</h2>
                        <span class="rounded-md border border-amber-500/40 px-3 py-1 text-sm text-amber-100">{{ $incomingChallenges->count() }}</span>
                    </div>

                    <div class="mt-4 space-y-3">
                        @forelse ($incomingChallenges as $challenge)
                            <article class="rounded-md border border-zinc-800 bg-zinc-950 p-4">
                                <div class="flex flex-wrap items-start justify-between gap-3">
                                    <div>
                                        <h3 class="font-semibold text-white">{{ $challenge->challengerCreature->name }}</h3>
                                        <p class="mt-1 text-sm text-zinc-400">
                                            {{ $challenge->challengerCreature->user->name }} вызывает {{ $challenge->defenderCreature->name }}
                                        </p>
                                        <p class="mt-1 text-xs text-zinc-500">Ответ до {{ $challenge->expires_at?->format('H:i:s') }}</p>
                                    </div>
                                    <a href="{{ route('arena.challenges.show', $challenge) }}" class="rounded-md border border-emerald-500/50 px-3 py-2 text-sm text-emerald-200 hover:bg-emerald-500/10">
                                        Открыть
                                    </a>
                                </div>
                            </article>
                        @empty
                            <p class="text-sm text-zinc-400">Нет входящих вызовов.</p>
                        @endforelse
                    </div>
                </div>
                ...
                <div class="rounded-md border border-zinc-800 bg-zinc-900 p-5">
                    <div class="flex items-center justify-between gap-3">
                        <h2 class="text-xl font-semibold text-white">Исходящие вызовы</h2>
                        <span class="rounded-md border border-sky-500/40 px-3 py-1 text-sm text-sky-100">{{ $outgoingChallenges->count() }}</span>
                    </div>

                    <div class="mt-4 space-y-3">
                        @forelse ($outgoingChallenges as $challenge)
                            <article class="rounded-md border border-zinc-800 bg-zinc-950 p-4">
                                <div class="flex flex-wrap items-start justify-between gap-3">
                                    <div>
                                        <h3 class="font-semibold text-white">{{ $challenge->defenderCreature->name }}</h3>
                                        <p class="mt-1 text-sm text-zinc-400">
                                            Ожидает ответа от {{ $challenge->defenderCreature->user->name }}
                                        </p>
                                        <p class="mt-1 text-xs text-zinc-500">До {{ $challenge->expires_at?->format('H:i:s') }}</p>
                                    </div>
                                    <a href="{{ route('arena.challenges.show', $challenge) }}" class="rounded-md border border-zinc-700 px-3 py-2 text-sm text-zinc-200 hover:bg-zinc-900">
                                        Статус
                                    </a>
                                </div>
                            </article>
                        @empty
                            <p class="text-sm text-zinc-400">Нет исходящих вызовов.</p>
                        @endforelse
                    </div>
                </div>
            </section>
        ...
        <section class="rounded-md border border-zinc-800 bg-zinc-900 p-5">
            <div class="flex flex-wrap items-start justify-between gap-3">
                <div>
                    <h2 class="text-xl font-semibold text-white">Поиск соперников</h2>
                    <p class="mt-1 text-sm text-zinc-400">Подбор учитывает уровень и power score. Подходящие боты создаются автоматически.</p>
                </div>
            </div>
            ...
            @else
                <form method="POST" action="{{ route('arena.search.store') }}" class="mt-5 grid gap-3 md:grid-cols-[1fr_auto]">
                    @csrf
